...
static const rsa_3072_public_key_t kPublicKey = {
    .n = {0x6a6a75e1, 0xa018ddc5, 0x687bb168, 0x8e8205a5, 0x7dbfffa7,
...
static const rsa_3072_constants_t kExpConstants = {
    .rr = {0xa3eb77fa, 0x9db9a2ac, 0x2c19d4ae, 0xfb5be1e7, 0xdd38f5fb,
...
rom_error_t precomp_test(void) {
  rsa_3072_constants_t actConstants;

  // Precompute constants
  RETURN_IF_ERROR(rsa_3072_precomp(&kPublicKey, &actConstants));

  // Check that RR matches expected value
  for (int i = 0; i < kRsa3072NumWords; i++) {
    CHECK(actConstants.rr.data[i] == kExpConstants.rr.data[i],
          "Mismatch at word %d of R^2: expected=0x%x, actual=0x%x", i,
          kExpConstants.rr.data[i], actConstants.rr.data[i]);
  }

  // Check that m0_inv matches expected value
  for (int i = 0; i < kOtbnWideWordNumWords; i++) {
    CHECK(actConstants.m0_inv[i] == kExpConstants.m0_inv[i],
          "Mismatch at word %d of m0_inv: expected=0x%x, actual=0x%x", i,
          kExpConstants.m0_inv[i], actConstants.m0_inv[i]);
  }
