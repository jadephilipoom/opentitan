...
rom_error_t spx_verify(const uint8_t *sig, const uint8_t *m, size_t mlen,
  ...
  HARDENED_RETURN_IF_ERROR(
      spx_hash_message(sig, pk, m, mlen, &ctx, mhash, &tree, &idx_leaf));
  sig += kSpxN;
  ...
  HARDENED_RETURN_IF_ERROR(fors_pk_from_sig(sig, mhash, &ctx, wots_addr, root));
  sig += kSpxForsBytes;

  // For each subtree..
  for (size_t i = 0; i < kSpxD; i++) {
    ...
    HARDENED_RETURN_IF_ERROR(
        wots_pk_from_sig(sig, (unsigned char *)root, &ctx, wots_addr, wots_pk));
    sig += kSpxWotsBytes;

    // Compute the leaf node using the WOTS public key.
    uint32_t leaf[kSpxNWords];
    HARDENED_RETURN_IF_ERROR(
        thash((unsigned char *)wots_pk, kSpxWotsLen, &ctx, wots_pk_addr, leaf));
