...
static void fors_gen_leafx1(unsigned char *leaf, const spx_ctx *ctx,
...
void fors_sign(unsigned char *sig, unsigned char *pk, const unsigned char *m,
  ...
  for (i = 0; i < SPX_FORS_TREES; i++) {
    ...
    treehashx1(roots + i * SPX_N, sig, ctx, indices[i], idx_offset,
               SPX_FORS_HEIGHT, fors_gen_leafx1, fors_tree_addr, &fors_info);
...
void fors_pk_from_sig(unsigned char *pk, const unsigned char *sig,
  ...
  for (i = 0; i < SPX_FORS_TREES; i++) {
    ...
    compute_root(roots + i * SPX_N, leaf, indices[i], idx_offset, sig,
                 SPX_FORS_HEIGHT, ctx, fors_tree_addr);
    sig += SPX_N * SPX_FORS_HEIGHT;
  }
