...
static void test_mlkem512_derand(void) {
  ...
  uint32_t key_a[ceil_div(kOtcryptoMlkem512SharedSecretBytes, sizeof(uint32_t))];
  uint32_t key_b[ceil_div(kOtcryptoMlkem512SharedSecretBytes, sizeof(uint32_t))];
...
static void test_mlkem768_derand(void) {
  ...
  uint32_t key_a[ceil_div(kOtcryptoMlkem768SharedSecretBytes, sizeof(uint32_t))];
  uint32_t key_b[ceil_div(kOtcryptoMlkem768SharedSecretBytes, sizeof(uint32_t))];
  ...
  CHECK_ARRAYS_EQ(key_a, key_b, ARRAYSIZE(key_a));
  CHECK_ARRAYS_EQ((unsigned char *)key_a, expected_key, kOtcryptoMlkem768SharedSecretBytes);
}
...
static void test_mlkem1024_derand(void) {
  ...
  uint32_t key_a[ceil_div(kOtcryptoMlkem1024SharedSecretBytes, sizeof(uint32_t))];
  uint32_t key_b[ceil_div(kOtcryptoMlkem1024SharedSecretBytes, sizeof(uint32_t))];
  ...
  CHECK_ARRAYS_EQ(key_a, key_b, ARRAYSIZE(key_a));
  CHECK_ARRAYS_EQ((unsigned char *)key_a, expected_key, kOtcryptoMlkem1024SharedSecretBytes);
}
...
static void test_mlkem512_randomized(void) {
  ...
  t0 = profile_start();
  CHECK_STATUS_OK(
      otcrypto_mlkem512_encapsulate(&pk_buf, ct_buf, &key_b_buf));
...
static void test_mlkem768_randomized(void) {
  ...
  uint32_t sk_blob[ARRAYSIZE(sk) * 2];
  memset(sk_blob, 0, sizeof(sk_blob));
  memcpy(sk_blob, sk, kOtcryptoMlkem768SecretKeyBytes);
  otcrypto_blinded_key_t sk_buf = {
    .config = kMlkem768SecretKeyConfig,
    .keyblob_length = sizeof(sk_blob),
    .keyblob = sk_blob, 
  ...
  uint32_t key_b_blob[ARRAYSIZE(key_b) * 2];
  memset(key_b_blob, 0, sizeof(key_b_blob));
  memcpy(key_b_blob, key_b, kOtcryptoMlkem768SharedSecretBytes);
  otcrypto_blinded_key_t key_b_buf = {
    .config = kMlkem768SharedSecretConfig,
    .keyblob_length = sizeof(key_b_blob),
    .keyblob = key_b_blob, 
  ...
  t0 = profile_start();
  CHECK_STATUS_OK(
      otcrypto_mlkem768_encapsulate(&pk_buf, ct_buf, &key_b_buf));
  profile_end_and_print(t0, "otcrypto_mlkem768_encapsulate");

  otcrypto_const_byte_buf_t ct_const_buf = {.data = ct, .len = sizeof(ct)};
  uint32_t key_a_blob[ARRAYSIZE(key_a) * 2];
  memset(key_a_blob, 0, sizeof(key_a_blob));
  memcpy(key_a_blob, key_a, kOtcryptoMlkem768SharedSecretBytes);
  otcrypto_blinded_key_t key_a_buf = {
    .config = kMlkem768SharedSecretConfig,
    .keyblob_length = sizeof(key_a_blob),
    .keyblob = key_a_blob, 
...
static void test_mlkem1024_randomized(void) {
  ...
  uint32_t sk_blob[ARRAYSIZE(sk) * 2];
  memset(sk_blob, 0, sizeof(sk_blob));
  memcpy(sk_blob, sk, kOtcryptoMlkem1024SecretKeyBytes);
  otcrypto_blinded_key_t sk_buf = {
    .config = kMlkem1024SecretKeyConfig,
    .keyblob_length = sizeof(sk_blob),
    .keyblob = sk_blob, 
  ...
  uint32_t key_b_blob[ARRAYSIZE(key_b) * 2];
  memset(key_b_blob, 0, sizeof(key_b_blob));
  memcpy(key_b_blob, key_b, kOtcryptoMlkem1024SharedSecretBytes);
  otcrypto_blinded_key_t key_b_buf = {
    .config = kMlkem1024SharedSecretConfig,
    .keyblob_length = sizeof(key_b_blob),
    .keyblob = key_b_blob, 
  ...
  t0 = profile_start();
  CHECK_STATUS_OK(
      otcrypto_mlkem1024_encapsulate(&pk_buf, ct_buf, &key_b_buf));
  profile_end_and_print(t0, "otcrypto_mlkem1024_encapsulate");

  otcrypto_const_byte_buf_t ct_const_buf = {.data = ct, .len = sizeof(ct)};
  uint32_t key_a_blob[ARRAYSIZE(key_a) * 2];
  memset(key_a_blob, 0, sizeof(key_a_blob));
  memcpy(key_a_blob, key_a, kOtcryptoMlkem1024SharedSecretBytes);
  otcrypto_blinded_key_t key_a_buf = {
    .config = kMlkem1024SharedSecretConfig,
    .keyblob_length = sizeof(key_a_blob),
    .keyblob = key_a_blob, 
