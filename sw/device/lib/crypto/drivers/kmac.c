...
static status_t set_prefix_regs(const unsigned char *func_name,
  ...
  hardened_mmio_write(kKmacBaseAddr + KMAC_PREFIX_0_REG_OFFSET, prefix,
...
static status_t read_state(size_t nwords, hardened_bool_t read_masked,
  ...
  if (launder32(read_masked) == kHardenedBoolTrue) {
    ...
    hardened_mmio_read(share0, kKmacStateShare0Addr, nwords);
    hardened_mmio_read(share1, kKmacStateShare1Addr, nwords);
  } else {
